...
import com.myproject.parking.lib.service.BookingService;
...
public class CheckOrderAllowPay implements BaseQueryLogic {

	private static final Logger LOG = LoggerFactory.getLogger(CheckOrderAllowPay.class);
	...
	public String process(HttpServletRequest request,HttpServletResponse response,String data, ObjectMapper mapper, String pathInfo) {
		...
		try {						
			...
			bookingService.checkBookingIdAllowPay(bookingVO);
			result = MessageUtils.handleSuccess("Booking Id : " + bookingVO.getBookingId() + " allowed to pay." , mapper);
